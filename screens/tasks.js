...
function TasksScreen({ navigation }) {
  function change(email, task) {
    changeTask(email, task)
      .then(() => {
        navigation.navigate("Home", { chosenTask: task });
      })
      .catch((error) => {
        alert(error);
      });
    ...
    <View style={style.view}>
      ...
      <View style={style.textPadding}>
        <Text style={style.text}>
          Please choose the task that you'd like to complete today from the 3
          selections below:
        </Text>
      </View>
      <TouchableOpacity
